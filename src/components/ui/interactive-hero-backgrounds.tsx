...
class X {
  ...
  size: ThreeSize = { width: 0, height: 0, wWidth: 0, wHeight: 0, ratio: 0, pixelRatio: 0 };
  onBeforeRender: (state: { elapsed: number; delta: number }) => void = () => {};
  onAfterResize: (size: ThreeSize) => void = () => {};

  constructor(config: ThreeBoilerplateConfig) {
    this.#config = config;
  ...
  #onIntersection(e: IntersectionObserverEntry[]) {
    this.#isAnimating = e[0]?.isIntersecting ?? false;
...
// --- Default Props ---
const defaultBallpitConfig: Omit<BallpitConfig, "colors"> = {
  count: 200,
...
export const BallpitBackground: React.FC<BallpitBackgroundProps> = ({
  ...
  const config = useMemo(() => {
    const colors =
      ballpitConfig.colors && ballpitConfig.colors.length
        ? ballpitConfig.colors
        : isDark
          ? darkColors
          : lightColors;
    ...
    <div className={cn("relative w-full h-screen overflow-hidden bg-background", className)}>
      ...
      <header className="relative z-10 w-full max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
        <div className="flex items-center justify-between gap-4">
          ...
          <nav className="hidden md:flex items-center gap-2 text-sm font-medium text-muted-foreground">
            <a href="#" className="hover:text-foreground px-3 py-2 transition-colors rounded-md">
              About
            </a>
            <a href="#" className="hover:text-foreground px-3 py-2 transition-colors rounded-md">
              Blog
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="bg-secondary/50 hover:bg-secondary shrink-0 p-2.5 rounded-full transition-colors"
      ...
      <main className="relative z-10 flex h-[calc(100%-100px)] items-center justify-center text-center px-4">
        <div className="max-w-3xl">
          <h1 className="text-4xl sm:text-5xl md:text-6xl text-foreground font-bold tracking-tighter">
            {heroTitle}
          </h1>
          ...
          >
            ...
            <button
              type="submit"
              className="w-full sm:w-auto bg-primary text-primary-foreground hover:bg-primary/90 px-6 py-3 rounded-full font-semibold transition-colors flex items-center justify-center gap-2 shrink-0"
            >
